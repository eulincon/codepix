...
func (t *TransactionUseCase) Regsiter(accountId string, amount float64, pixKeyTo string, pixKeyKindTo string, description string) (*model.Transaction, error) {
...
func (t *TransactionUseCase) Confirm(transactionId string) (*model.Transaction, error) {
	...
	if err != nil {
		log.Println("updating transaction status to confirmed", transactionId)
		return nil, err
	}
...
func (t *TransactionUseCase) Complete(transactionId string) (*model.Transaction, error) {
	...
	err = t.TransactionRepository.Save(transaction)
	if err != nil {
		log.Println("failed updating transaction status to completed", transactionId)
		return nil, err
	}
	return transaction, nil
}
...
func (t *TransactionUseCase) Error(transactionId string, reason string) (*model.Transaction, error) {
	...
	err = t.TransactionRepository.Save(transaction)
	if err != nil {
		log.Println("failed updating transaction status to error", transactionId)
		return nil, err
	}
